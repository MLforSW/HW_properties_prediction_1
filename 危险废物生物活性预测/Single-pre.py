...
import gc
# 加载数据
data = pd.read_csv('single-pre-update.csv')
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# 从数据中提取SMILES字符串和标签
smiles = data['SMILES']
labels = data['ASSAY_OUTCOME']
...
# 定义模型
Attentive_FP = AttentiveFPPredictor(node_feat_size=74, edge_feat_size=12, num_layers=2, num_timesteps=2, n_tasks=1)
GAT = GATPredictor(in_feats=74, classifier_hidden_feats=200, n_tasks=1, predictor_hidden_feats=200)
GCN = GCNPredictor(in_feats=74, classifier_hidden_feats=200, n_tasks=1, predictor_hidden_feats=200)
MPNN = MPNNPredictor(node_in_feats=74, node_out_feats=128, edge_hidden_feats=128, edge_in_feats=12, n_tasks=1)
NF = NFPredictor(in_feats=74, n_tasks=1, predictor_hidden_size=200)
PAGTN = PAGTNPredictor(node_in_feats=74, node_out_feats=128, node_hid_feats=128, edge_feats=12, depth=5)
Weave = WeavePredictor(node_in_feats=74, edge_in_feats=12, gnn_hidden_feats=64, graph_feats=64, num_gnn_layers=3)
...
def train_and_evaluate(model, device, train_loader, valid_loader, test_loader, optimizer, scheduler, loss_fn,
    ...
    log_file = f'{model_name}_trainlog.csv'
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if valid_auc > best_valid_auc:
                ...
                torch.save(model.state_dict(), f'single_best_model_{model_name}.pth')
    ...
    # 加载最佳模型进行测试集评估
    model.load_state_dict(torch.load(f'single_best_model_{type(model).__name__}.pth'))
    model.eval()
    test_labels = []
    test_preds = []
    with torch.no_grad():
